...
annotation_folder = tkFileDialog.askdirectory(initialdir= "/home/eebrandt/projects/dissertation/chapter_1/male_only/data/", title = "Choose the folder that contains annotations")
...
#something to implement later, if desired
#plotscth = tkMessageBox.askyesno("Peak Graphs", "Do you want to see plots for scrape and thump frequencies?")
plotwav = tkMessageBox.askyesno("Peak Graphs", "Do you want to see plots and have the option to change buzz peaks?")
if wav_folder == "":
	tkMessageBox.showerror(
	"Fatal Error",
	...
	if os.path.isdir(annotation_folder + "/" + individual):
		...
		for trial in trials:
			...
			if os.path.isdir(annotation_folder + "/" + individual + "/" + trial) and os.path.isfile(labelfilename):
				...
				an_info.append(animal_info["treatment"][trialindex])
				an_info.append(animal_info["rank"][trialindex])
				an_info.append(animal_info["date"][trialindex])
				# calculates temperature in celsius from the given temperature in farenheit
				tempf = (float(animal_info["temp"][trialindex]))
				tempc = (tempf - 32) * 5.0/9.0
				an_info.append(tempc)
				an_info.append(animal_info["weight"][trialindex])
				an_info.append(animal_info["ceph_width"][trialindex])
				...
				# runs peak frequency analysis. This is divided into two different analyses. The reason they are separated is 	
				# lost to memory at this point
				#1: scrape and thump peak frequency, using the "simple peaks" function. 
				#2: buzz peak and fundamental frequencies using "get peaks"
				
				#for this first analysis, we're only going through scrapes and thumps
				featuretype = 0
				nonbuzz_frq = []
				nonbuzz_peak = []
				while featuretype < 2:
					...
					while readvar < len(cfg.lengths_output[featuretype][5]):
						try:
							...
							print "\a"
						#puts both frequency an peak information into arrays
				...
				# now we're going to do the same thing with buzzes
				readvar = 0
				peakarray = np.zeros((2, lenbuzz))
				fundarray = np.zeros((2, lenbuzz))
				while readvar < lenbuzz:
					cfg.plotinfo = str(trial) + " buzz " + str(readvar +1)
					print cfg.plotinfo
					...
					try:
						...
						print "getfreq complete"
						# performs peak analysis, using the "getpeaks" function
						vib.getpeaks(cfg.fft_dat[0], cfg.fft_dat[1], .01, plotwav, smooth = 5, plot_title = str(trial) + " buzz " + str(readvar + 1))											
					...
					#writes fundamental and peak frequencies to arrays. Note that we're sidestepping some of the previous code
					#used to get the peaks
					fundarray[0][readvar] = cfg.fund
					peakarray[0][readvar] = cfg.maxpeak
				
					readvar = readvar + 1

				#plots durations if user requests them
				if duration_plots:
				...
				npoutput = np.zeros((30, Maxlen), dtype="S20")
				...
				# and its duration.  There are commented-out lines for the amplitudes of the different values, but I haven't been using them. TODO: possibly clean this up with a loop?
				...
				durations_output_header = ["tape-video", "complete?", "individual", "treatment", "rank", "date", "temperature (C)", "weight", "ct_width", "scrape_pos", "scrape_dur", "scrape_rms", "thump_pos", "thump_dur", "thump_rms", "buzz_pos", "buzz_dur", "buzz_rms", "srate_pos", "srate_dur", "srate_num", "scrape_freq", "thump_freq" , "buzz_fund", "buzz_peak", "comments"]
				...
				fl = open(annotation_folder +"/"+ individual + "/" + trial + "/" + trial +"_" + "alldata" + "_" + timestamp + '.csv', 'w')
