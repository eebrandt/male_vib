		...
		for trial in trials:
			...
			if os.path.isdir(trial_folder):
				...
				if not csvs:
					...
					break
				#this is an old feature to find the  most recent data file for each trial. Since we're going through and hand-verifying each frequency, it's been changed so that the file has to have "_final" at the end. Now we don't care with the most recent file is, just which one is the final one.
				#creation_time = []
				...
				# old file-opening arguments and such that no longer work
				...
				#kwargs = dict(delimiter=",",
               				#dtype= "S10, S10, S10, S10, S10, S10, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, f20, S100",
              				#names= "video, complete, individual, treatment, rank, date, temperature, weight, ct_width, scrape_pos, scrape_dur, scrape_rms, thump_pos, thump_dur, thump_rms, buzz_pos, buzz_dur, buzz_rms, srate_pos, srate_dur, srate_num, scrape_peak, thump_peak, buzz_fund, buzz_peak, comments",
					#skip_header=1,
					#usecols = np.arange(0,26))
				individualarray = trial_folder.split("/")
				# load the data file once we know which is the proper one 
				cols = range(0,26)
				data_file = pd.read_csv(trial_folder + "/" + individualarray[-1] + "_alldata_final.csv",sep=',', dtype = str, header = 0, usecols =cols, na_filter=False)
				...
				for feature in feature_type:
					...
					while quartile_count < 5:
						...
						for item in data_file[feature + "_pos"]:
							...
							if item != "" and (quartile_count * .25) > float(item) > ((quartile_count * .25) - .25):
						...
						if quartile_durs:
							mean_quartile.append(np.mean(map (float, quartile_durs)))
							mean_qrms.append(np.mean(map(float, quartile_rms)))
						else:
					...
					if average_count:
						mean_quartile.append(np.mean(map(float,average_count)))
						mean_qrms.append(np.mean(map(float, average_rms)))
					else:
						mean_quartile.append(" ")
						mean_qrms.append(" ")
				...
				while quartile_count < 5:
					...
					for item in data_file["srate_pos"]:
						...
						if item != "" and  (quartile_count * .25) > float(item) > ((quartile_count * .25) - .25):
							sdur = sdur + float(data_file["srate_dur"][readvar])
							scount = scount + float(data_file["srate_num"][readvar])
						readvar = readvar + 1
				...
				while quartile_count < 5:
					...
					for item in data_file["buzz_pos"]:
						...
						if  data_file["buzz_fund"][readvar] != "" and float(data_file["buzz_fund"][readvar]) > 0 and  (quartile_count * .25) > float(item) > ((quartile_count * .25) - .25) and float(data_file["buzz_peak"][readvar]) > 1:				
							peak_count.append(data_file["buzz_peak"][readvar])
							fund_count.append(data_file["buzz_fund"][readvar])
							allpeak.append(data_file["buzz_peak"][readvar])
							allfund.append(data_file["buzz_fund"][readvar])
						...
						readvar = readvar + 1	
					if peak_count:
						quartile_peak[quartile_count - 1] = np.mean(map(float,peak_count))
					else:
						quartile_peak[quartile_count - 1] = " "
					if fund_count:
						quartile_fund[quartile_count -1] = np.mean(map(float,fund_count))
					else:
				...
				if allfund:
					quartile_fund[4] = np.mean(map(float, allfund))
					quartile_peak[4] = np.mean(map(float, allpeak))
				else:
					quartile_fund[4] = (" ")
					quartile_peak[4] = (" ")
				...
				while featurecount < 2:
					...
					while quartile_count < 5:	
						...
						for item in data_file[colpos]:
							if data_file[colpos][readvar] != "" and data_file[colpos][readvar] > 0 and  (quartile_count * .25) > float(item) > ((quartile_count * .25) - .25):
						...
						# makes sure there's something in the quartile_freq array.  It will throw an error (but not abort the program) if this isn't included
						if peak_count:
							nbquartile_peak[quartile_count - 1] = np.mean(map(float, peak_count))
						else:
							nbquartile_peak[quartile_count - 1] = " "
						if freq_count:
							quartile_freq[quartile_count - 1] = np.mean(map(float,freq_count))
						else:
					...
					# also makes sure there's something in peak_count, so we don't get weird errors.  Note that we add empty string if it doesn't have a number so things don't get appended over necessary blanks.
					if allfreq:
						quartile_freq[4] = np.mean(map(float,allfreq))
						nbquartile_peak[4] = np.mean(map(float,allpeak))
					else:
						quartile_freq[4] = (" ")
						nbquartile_peak[4] = (" ")
					nonbuzzfreq.append(quartile_freq) 
					nonbuzzpeak.append(nbquartile_peak)
				...
				row = [data_file["tape-video"][0], data_file["complete?"][0], data_file["individual"][0], data_file["treatment"][0] ,data_file["rank"][0], data_file["date"][0], data_file["temperature (C)"][0], weight, ct, allmeanfeatures[0][0], allmeanfeatures[0][1], allmeanfeatures[0][2], allmeanfeatures[0][3], allmeanfeatures[0][4], allmeanrms[0][0], allmeanrms[0][1], allmeanrms[0][2],allmeanrms[0][3], allmeanrms[0][4], allmeanfeatures[1][0], allmeanfeatures[1][1], allmeanfeatures[1][2], allmeanfeatures[1][3], allmeanfeatures[1][4],allmeanrms[1][0],allmeanrms[1][1],allmeanrms[1][2],allmeanrms[1][3],allmeanrms[1][4], allmeanfeatures[2][0], allmeanfeatures[2][1], allmeanfeatures[2][2], allmeanfeatures[2][3], allmeanfeatures[2][4], allmeanrms[2][0],allmeanrms[2][1],allmeanrms[2][2],allmeanrms[2][3],allmeanrms[2][4], rates[0], rates[1], rates[2], rates[3], rates[4], nonbuzzfreq[0][0], nonbuzzfreq[0][1], nonbuzzfreq[0][2],  nonbuzzfreq[0][3], nonbuzzfreq[0][4], nonbuzzfreq[1][0], nonbuzzfreq[1][1], nonbuzzfreq[1][2], nonbuzzfreq[1][3], nonbuzzfreq[1][4], quartile_fund[0], quartile_fund[1], quartile_fund[2], quartile_fund[3], quartile_fund[4], quartile_peak[0], quartile_peak[1], quartile_peak[2], quartile_peak[3], quartile_peak[4], data_file["comments"][0]]
