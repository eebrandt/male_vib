...
dat <- read.csv("female_time_final.csv")

# CLEAN DATA
dat <- subset(dat, dat$round.number == "1")
dat$round.number <- NULL
dat <- subset(dat, dat$outcome != "redo")
dat$comments <- NULL
...
dat$female.ID <- factor(dat$female.ID)
dat$male.ID <- factor(dat$male.ID)
...
mod.list[[1]] <- gamm(sidle.number~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[2]] <- gamm(time.sidling~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[3]] <- gamm(percent.sidling~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[4]] <- gamm(sidle.length~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[5]] <- gamm(mov.number.sidle~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[6]] <- gamm(mov.length~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[7]] <- gamm(mov.time~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
mod.list[[8]] <- gamm(mov.percent.sidle~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
#mod.list[[9]] <- gamm(female.attacks~ s(temp, k=knots.val), random=list(male.ID=~1), family=Gamma(link="log"), data=dat)
#mod.list[[10]] <- gamm(female.escapes ~ s(temp, k=knots.val), random=list(male.ID=~1, female.ID=~1), family=Gamma(link="log"), data=dat)
